...
export function leoConfiguration() {
    ...
    return {
        ...
        loadSavedStates,
        addSavedState,
        addOrUpdateSavedState,
        fetchStatesByUrlAndMethod,
        removeState,
        removeOption,
    ...
    function upsertOption(state, name, active) {
        let statesStatus = self.storage.getStates();
        statesStatus[state] = {
            name: name || findStateOption(state).name,
            active: active
        };

        self.storage.setStates(statesStatus);
    ...
    function activateJsonpMObserver() {
        if (self._jsonpMutationObservers) {
            if (!fetchStates().some(state => state.verb === 'JSONP' && state.active)) {
                self._jsonpMutationObservers.forEach(mutationObserver => mutationObserver && mutationObserver.disconnect());
                delete self._jsonpCallbacks;
                delete self._jsonpMutationObservers;
            }
            return;
        }
        ...
        self._jsonpMutationObservers = targets.map((target) => {
        ...
        targets.forEach((target, index) => self._jsonpMutationObservers[index].observe(target, config));
    }
    ...
    function fetchStates() {
        const activeStates = self.storage.getStates();
    ...
    function toggleActivateAll(flag: boolean) {
        ...
        const statuses = statesStatus.reduce((obj, s) => {
            ...
            , {});
        self.storage.setStates(statuses);
        return statesStatus;
    }
    ...
    function addScenario(scenario, fromLocal: boolean = false) {
        if (scenario && typeof scenario.name === 'string') {
            if (fromLocal) {
                const scenarios = self.storage.getScenarios();
                scenarios.push(scenario);
                self.storage.setScenarios(scenarios);
            } else {
    ...
    function getScenarios() {
        const scenarios = self.storage.getScenarios().map((scenario: any) => scenario.name);
    ...
    function getScenario(name: string) {
        ...
        if (_scenarios[name]) {
            ...
            states = self.storage.getScenarios()
    ...
    function loadSavedStates() {
        _savedStates = self.storage.getSavedStates();
        addStates(_savedStates, true);
    }

    function addSavedState(state) {
        _savedStates.push(state);
        self.storage.setSavedStates(_savedStates);
        addState(state, true);
    }
    ...
    function toggleConsoleOutput() {
        ...
        self.storage.setConsoleOutput(_consoleOutputEnabled);
    }

    function logRequestToConsole(req: INetworkRequest) {
        if (_consoleOutputEnabled === null) {
            _consoleOutputEnabled = self.storage.getConsoleOutput();
        }
